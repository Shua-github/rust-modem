...
static void
run_ready (QfuDownloadReseter *reseter,
           GAsyncResult       *res,
           ReseterOperation   *operation)
{
    GError *error = NULL;

    if (!qfu_download_reseter_run_finish (reseter, res, &error)) {
        g_printerr ("error: download reseter operation finished: %s\n", error->message);
        g_error_free (error);
    } else {
...
static gboolean
operation_download_reseter_run (QfuDownloadReseter *reseter)
{
    ReseterOperation operation = {
        .loop        = NULL,
        .cancellable = NULL,
    ...
    g_assert (QFU_IS_DOWNLOAD_RESETER (reseter));
    ...
    /* Run! */
    qfu_download_reseter_run (reseter, operation.cancellable, (GAsyncReadyCallback) run_ready, &operation);
...
gboolean
qfu_operation_reset_download_run (QfuDeviceSelection *device_selection)
{
    QfuDownloadReseter *reseter = NULL;
    gboolean            result;

    reseter = qfu_download_reseter_new (device_selection);
    result = operation_download_reseter_run (reseter);
